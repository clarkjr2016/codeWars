...
function chineseZodiac(year) {
  const epochYear = 1924;
  const gap = year - epochYear;
  const animalCycle = Math.floor(gap % 12);
  const elementCycle = Math.floor(gap % 10);
  ...
  const element = ["Wood", "Fire", "Earth", "Metal", "Water"];

  return `${element[elementCycle]} ${animals[animalCycle]}`;
}
...
Parameters - A year
Return - The appropriate element and animal associated with the Chinese Year
Example - (1924) -> "Wood Rat"
Psuedocode - 

- Use the year and subtract 1924 from it. 
- Find th 
